...
describe('upgradeDependencies()', () => {
	...
	it('upgrades dependencies and removes/reinstalls them', async () => {
		...
		expect(mockedShellInstance.run).toHaveBeenCalledWith('npm outdated --all --json', false);
		...
		expect(mockedShellInstance.run).toHaveBeenCalledWith('rm -f package-lock.json');

		// 5) Check that "npm update --save" was called via shell.stdout:
		expect(mockedShellInstance.stdout).toHaveBeenCalledWith('npm update --save');
	...
	it('handles case when no dependencies are outdated', async () => {
		...
		mockedShellInstance.run.mockImplementationOnce(async (command: string) => {
			if (command === 'npm outdated --all --json') {
				return { code: 0, signal: '', stdout: '{}', stderr: '' };
			}
			return { code: 0, signal: '', stdout: '', stderr: '' };
